...
from d9d.module.parallelism.model.qwen3_dense import (
    parallelize_qwen3_dense_for_classification,
)
...
from d9d_test.modules.model.qwen3_dense.util import check_cls_model_grad, clone_cls_model_weights

_PAD_TOKEN_ID = 99


def build_hf_cls_model(num_labels=3) -> Qwen3ForSequenceClassification:
    torch.manual_seed(131231)

    hf = Qwen3ForSequenceClassification(
        Qwen3Config(
            vocab_size=100,
            num_hidden_layers=2,
            hidden_size=128,
...
def build_my_cls_model(stage: PipelineStageInfo, checkpointing: bool, num_labels=3) -> Qwen3DenseForClassification:
...
def _build_distributed_cls_model(
    ...
    parallelize_qwen3_dense_for_classification(dist_ctx, model, stage)
