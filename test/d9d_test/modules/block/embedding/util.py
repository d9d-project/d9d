...
def clone_embeddings(my: SplitTokenEmbeddings, hf: nn.Embedding):
    size_per_part = {name: part.num_embeddings for name, part in my.token_embedding.items()}
    prepend_size = 0
...
def check_embeddings_grad(my: SplitTokenEmbeddings, hf: nn.Embedding):
    my_grad = torch.cat([my.token_embedding[part_name].weight.grad for part_name in my._split_order], dim=0)
