...
def clone_lm_head(my: SplitLanguageModellingHead, hf: nn.Linear):
    size_per_head = {name: head.out_features for name, head in my.lm_head.items()}
    prepend_size = 0
...
def check_lm_head_grad(my: SplitLanguageModellingHead, hf: nn.Linear):
    my_grad = torch.cat([my.lm_head[part_name].weight.grad for part_name in my._split_order], dim=0)
