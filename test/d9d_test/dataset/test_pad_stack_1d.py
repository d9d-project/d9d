...
def test_errors():
    ...
    with pytest.raises(ValueError, match="pad_to_multiple_of"):
        pad_stack_1d([torch.tensor([1])], pad_value=0, padding_side=PaddingSide1D.right, pad_to_multiple_of=0)

    with pytest.raises(ValueError, match="pad_to_multiple_of"):
        pad_stack_1d([torch.tensor([1])], pad_value=0, padding_side=PaddingSide1D.right, pad_to_multiple_of=-1)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([[1, 2], [3]], 0, PaddingSide1D.left, 3, (2, 3), [[0, 1, 2], [0, 0, 3]]),
        # Case 7: Different Pad Value
        ([[1], [2, 3]], -1, PaddingSide1D.right, None, (2, 2), [[1, -1], [2, 3]]),
    ],
)
def test_ok(
    ...
    result = pad_stack_1d(items, pad_value=pad_val, padding_side=side, pad_to_multiple_of=multiple_of)
