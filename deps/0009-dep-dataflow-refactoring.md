...
def forward_backward(self, pack: MicrobatchPack) -> None:
    ...
    callback = LossComputer(self._task, self._schedule, self._grad_manager, self._metrics)  # fresh per step
    self._pipeline.schedule.step(inputs_microbatches, kwargs_microbatches, callback)
```

`configure` and `step` are a single call: the schedule receives the already-built per-microbatch inputs and lazily (re)compiles its program and (re)allocates buffers inside `step`, guarded by caches — the program keyed on the microbatch count, the buffers keyed on the shapes/dtypes of *every* microbatch in the pack. Buffers are sized **per microbatch** (each stage runs `infer_stage_*` once per microbatch, so microbatch `i`'s P2P receive buffers match microbatch `i`), so the microbatches in a pack may genuinely differ in shape; the common constant-shape run still compiles and allocates exactly once. Microbatches must share the same input *names* (the program is structural), but not shapes. The operator returns nothing; loss/weight and metrics are accumulated per microbatch through the loss callback (see below).
...
Aggregation no longer flows through the state either: the per-microbatch callback hands the task each microbatch's outputs as produced, and the task accumulates directly into objects that already do so — loss/weight into the `GradientManager` (`add_loss_with_weight`, a running `WeightedMeanMetric`), metrics into their own `Stateful` accumulators — so no global tensor is ever materialized. The callback is supplied to `schedule.step(...)` per step rather than baked into the schedule at construction: the task operator builds a fresh `LossComputer` (bound to the task, schedule, gradient manager, and metrics it already holds) each step and passes it in.
...
- **`PipelineSchedule` API changes**: `configure_buffers` + `step` collapse into a single `step(inputs_microbatches, kwargs_microbatches, callback)` — the callback is now supplied per step rather than at schedule construction; `PipelineShardingSpec` and `BuildForwardInputsResult.pipeline_sharding_spec` are removed.
