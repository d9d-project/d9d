...
When d9d itself samples the rollout, `RouterReplayRecorder` captures the selection of every MoE layer during a forward pass and assembles it into the `replay_indices` tensor:

```python
from d9d.module.block.moe import RouterReplayRecorder

with RouterReplayRecorder().install(model) as recorder, torch.no_grad():
