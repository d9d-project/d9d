...
Qwen3p5DecoderLayer = Qwen3p5FullAttentionLayer | Qwen3p5LinearAttentionLayer
...
class Qwen3p5Model(
    ...
    """The Qwen3.5 dense Transformer Decoder backbone.
    ...
    def __init__(
        ...
        """Constructs the Qwen3p5Model object.
        ...
        layers = nn.ModuleDict(
            {
                str(layer_idx): (
                    Qwen3p5FullAttentionLayer(params=params.layer)
                    if params.is_full_attention_layer(layer_idx)
                    else Qwen3p5LinearAttentionLayer(params=params.layer)
                )
                for layer_idx in range(layer_start, layer_end)
            }
        )
        self.layers: Mapping[str, Qwen3p5DecoderLayer] = cast(Mapping[str, Qwen3p5DecoderLayer], layers)
